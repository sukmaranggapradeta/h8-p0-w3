...
function tentukanDeretGeometri(arr) {
    ...
    var nilaiPengkali = 0;
    var jumlahNilaiPengkali = 0;
    for (var y = 0 ; y < (arr.length-1) ; y++){
      var angkaPertama = arr[y];
      var angkaKedua = arr[y+1];
      nilaiPengkali = (angkaKedua / angkaPertama);
      jumlahNilaiPengkali += nilaiPengkali
    }
    //console.log(jumlahNilaiPengkali + " " + pembagi + " " +nilaiPengkali)
    if ((jumlahNilaiPengkali/pembagi) === nilaiPengkali){
      return true;
    }else{
      return false;
    }
